...
#cols =    ['账号', '分行', '业务代码', '利率', '起息日', '到期日', '计息标志', '计息方式代码', '浮动类型', '浮动值', '存期', '原币金额', '货币代码', '折人民币金额', '二级科目代码']  
...
#    df['下次付息日']          = df.apply(lambda row: cal_base_05(row['计息方式'],row['到期日_EY']), axis=1)
    ...
    df['折人民币金额(test)']  = df.apply(lambda row: round(-row['原币金额']*row['汇率'],2), axis=1)
    print("--- %s seconds ---" % (time.time() - start_time))
    ...
    for sujcode in sujcodes:
        ...
        dff.to_excel(r'.\result\res_%s.xlsx'%sujcode, index=False)
        print("--- %s seconds ---" % (time.time() - start_time))
